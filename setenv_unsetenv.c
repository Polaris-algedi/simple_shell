...
void exit_shell(char **commands);
void print_env();
int exe_built_ins(char **commands);
int _isdigit(int c);
...
void exit_shell(char **commands)
{
    ...
    {
        /* work on this later*/
        printf("/bin/sh: 1: exit: Illegal number: 50x0");
        exit(2);
    }
    else
        exit(ext_vl);
...
list_t *array_To_SLL()
{
    ...
    for (i = 0; environ[i] != NULL; i++)
    {
        ...
        if (i == 0)
            head = new;
        var_len = strlen(environ[i]) + 1;  /* +1 for null-terminator */
